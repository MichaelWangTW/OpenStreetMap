...
import bearmaps.PathAlgorithm.streetmap.AStarSolver;
...
public class Router {
    ...
    public static List<NavigationDirection> routeDirections(AugmentedStreetMapGraph g, List<Long> route){
        List<NavigationDirection> path = new LinkedList<>();
        if(route.size() <= 1) {
            return path;
        }
...
//
//        double prevBearing = NavigationDirection.bearing(g.lon(start), g.lon(route.get(1)),
//                g.lat(start), g.lat(route.get(1)));
//        double currDist = 0.0;
//        String currWayName = null;
//
//
...
//
//           double currBearing = NavigationDirection.bearing(g.lon(prev), g.lon(cur),
//                   g.lat(prev), g.lat(cur));
//           int nextDirection = NavigationDirection.getDirection(prevBearing,currBearing);
//
...
//        }
        ...
        int prevDirection = 1;
//        double prevdistance = g.estimatedDistanceToGoal(prev, cur);
        double prevBearing = NavigationDirection.bearing(g.lon(prev), g.lon(cur), g.lat(prev), g.lat(cur));

        NavigationDirection tmp = new NavigationDirection();
        tmp.direction = NavigationDirection.START;
        tmp.way = Router.getWayName(g,prev,cur);
        tmp.distance = g.estimatedDistanceToGoal(prev, cur);
        path.add(tmp);
        ...
        for (int i = 2; i < route.size(); i++) {
            ...
            double currBearing = NavigationDirection.bearing(g.lon(prev), g.lon(cur), g.lat(prev), g.lat(cur));
            int currDirection = NavigationDirection.getDirection(prevBearing, currBearing);
            double currDistance = g.estimatedDistanceToGoal(prev, cur);

            if (prevDirection == currDirection) {
                path.get(index).distance += currDistance;
            } else {
                NavigationDirection nav = new NavigationDirection();
                nav.distance = currDistance;
                nav.direction = currDirection;
                nav.way = Router.getWayName(g,prev,cur);
                path.add(nav);
//                prevdistance = currDistance;
    ...
    private static String getWayName(AugmentedStreetMapGraph g,Long from,Long to){
        for (String begin: Router.wayNamesRepo(g,from)){
            for (String end: Router.wayNamesRepo(g,to)){
                if(begin.equals(end)){
                    return begin;
                }
            }
    ...
    private static Set<String> wayNamesRepo(AugmentedStreetMapGraph g, Long id){
        List<WeightedEdge<Long>> neighbors = g.neighbors(id);
        Set<String> repo = new HashSet<>();

        for (WeightedEdge<Long> w:neighbors){
            if(!repo.contains(w.getName()) && w.from().equals(id)){
                repo.add(w.getName());
            }
        }
        return repo;
        ...
        private static int getDirection(double prevBearing, double currBearing) {
            ...
            if ((currBearing > prevBearing && absDiff < 180.0)
                ...
                if (numInRange(absDiff, 15.0, 30.0) || absDiff > 330.0) {
                    // bearmaps.Mapample of high abs diff is prev = 355 and curr = 2
